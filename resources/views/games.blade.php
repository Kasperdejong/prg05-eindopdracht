<x-app-layout>
    <x-layout title="games">
        <div class="container mx-auto my-10">
            <h1 class="text-center text-4xl font-bold mb-10">The Game Vault</h1>

            <div class="mb-6 flex justify-center space-x-4">
                @foreach ($genres as $genre)
                    <a href="{{ route('games.index', ['genre_id' => $genre->id, 'search' => request('search')]) }}" class="bg-indigo-500 no-underline text-white py-2 px-4 rounded-lg hover:bg-indigo-600">
                        {{ $genre->title }}
                    </a>
                @endforeach
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-8">
                <a href="{{ route('games.create') }}"
                   class="inline-block bg-indigo-600 no-underline text-white font-bold py-4 px-8 rounded-lg text-xl hover:bg-indigo-700 transition duration-300 ease-in-out shadow-lg mb-4">
                    Create New Game
                </a>
